...
每个命中会带前后 `contextEvents` 条上下文事件：输出行以 `>>` 标记命中本身，缩进两格的行为上下文（去重、按 seq 排序）。

- **相关度排序**：长词（稀有）与出现密度高的命中靠前，同分时新的优先；中文长词按 2-4 字片段兜底匹配——压缩改写导致长句原文不连续时仍能召回
- **长事件截断**：只保留命中位置附近的内容（以命中为中心各取一半预算），超出部分标注「…(省略N字符)…」
...
`maxResults`（每次调用命中数上限）、`maxCharsPerEvent`（每个事件文本字符上限，默认 20000）与 `contextEvents`（每个命中前后各带多少条上下文事件，默认 2）为必填部署配置，可在挂载行的 `config` 中调整。
