// Real Loader composition through cordis.yml: the remind tool must boot through
...
function seededSession(): Session {
  const seed: SessionEvent[] = [
    {
      ...
      data: {
        ...
        source: { kind: 'user' },
        content: [{ type: 'text', text: 'pre-compaction user question about remind' }],
      },
      surfaceOp: 'append',
    },
    {
    ...
    },
    {
      ...
      data: { compactionId: CompactionId('remind-test'), turn: null },
    },
    {
      type: 'compaction/summary', seq: 3, time: 4,
      data: {
        compactionId: CompactionId('remind-test'),
  ...
  return Session.create(SessionId('remind-loader-agent'), seed)
}
...
async function remind(ctx: Context, owner: Agent, arguments_: Record<string, unknown>) {
  return ctx.tools.execute({
    signal: new AbortController().signal,
    callId: CallId('remind-test'),
    name: 'remind',
...
describe('tool-remind real Loader composition through cordis.yml', () => {
  it('registers the remind schema with the compaction-remind surface filter', async () => {
    const ctx = await boot()
    const schema = ctx.tools.schemas().find(s => s.name === 'remind')
  ...
  it('reminds compaction-shadowed pre-compaction content by keyword', async () => {
    const ctx = await boot()
    const owner = agent(ctx, seededSession())
    const result = await remind(ctx, owner, { query: 'pre-compaction' })
    ...
    expect(text).toContain('pre-compaction user question about remind')
  ...
  it('matches only events containing every space-separated keyword', async () => {
    const ctx = await boot()
    const owner = agent(ctx, seededSession())
    const both = await remind(ctx, owner, { query: 'pre-compaction question', max_results: 10 })
    ...
    const none = await remind(ctx, owner, { query: 'pre-compaction summary', max_results: 10 })
    expect(none.isError).toBe(false)
    expect(resultText(none)).toContain('Recalled 0 matching event(s)')
  }, 30_000)

  it('excludes events of the current step from remind', async () => {
    const ctx = await boot()
    const owner = agent(ctx, seededSession())
    const result = await remind(ctx, owner, { query: 'in-flight' })
    expect(result.isError).toBe(false)
  ...
  it('filters by the optional surfaces argument', async () => {
    const ctx = await boot()
    const owner = agent(ctx, seededSession())
    const result = await remind(ctx, owner, { query: 'pre-compaction', surfaces: ['current'], max_results: 10 })
    expect(result.isError).toBe(false)
    expect(resultText(result)).toContain('Recalled 0 matching event(s)')
  }, 30_000)
  ...
  it('caps hits at max_results and clamps oversized requests', async () => {
    ...
    // the single slot.
    const capped = await remind(ctx, owner, { query: 'pre-compaction', max_results: 1 })
    ...
    const oversized = await remind(ctx, owner, { query: 'pre-compaction', max_results: 99 })
  ...
  it('ranks hits by term density, newest first on ties', async () => {
    const ctx = await boot()
    const session = Session.create(SessionId('remind-rank'), [
    ...
    const result = await remind(ctx, owner, { query: 'beta', max_results: 1 })
    expect(result.isError).toBe(false)
    const text = resultText(result)
  ...
  it('recruits CJK partial matches via n-grams', async () => {
    const ctx = await boot()
    const session = Session.create(SessionId('remind-gram'), [
    ...
    const owner = agent(ctx, session)
    const result = await remind(ctx, owner, { query: '压缩历史内容' })
    expect(result.isError).toBe(false)
    const text = resultText(result)
  ...
  it('truncates long event texts around the first match', async () => {
    ...
    const session = Session.create(SessionId('remind-truncate'), [
    ...
    const owner = agent(ctx, session)
    const result = await remind(ctx, owner, { query: 'keyword' })
    expect(result.isError).toBe(false)
    const text = resultText(result)
  ...
  it('windows long texts below the per-event cap (compact 2400-char budget)', async () => {
    ...
    const session = Session.create(SessionId('remind-window'), [
  ...
  it('remind results do not echo themselves, call arguments remain facts', async () => {
    const ctx = await boot()
    const session = Session.create(SessionId('remind-echo'), [
      { type: 'tool/call', seq: 0, time: 1, data: { turn: 1, step: 1, callId: CallId('echo-0'), name: 'remind', arguments: JSON.stringify({ query: 'other' }) } },
    ...
    const result = await remind(ctx, owner, { query: 'needle' })
    expect(result.isError).toBe(false)
    const text = resultText(result)
    expect(text).toContain('Recalled 1 matching event(s)')
    ...
    const byQuery = await remind(ctx, owner, { query: 'other' })
  ...
  it('tells the agent when the session never compacted', async () => {
    const ctx = await boot()
    const session = Session.create(SessionId('remind-fresh'), [
    ...
    const owner = agent(ctx, session)
    const result = await remind(ctx, owner, { query: 'nonexistent-thing' })
    expect(result.isError).toBe(false)
  ...
  it('rejects queries longer than 200 characters', async () => {
    const ctx = await boot()
    const owner = agent(ctx, seededSession())
    const result = await remind(ctx, owner, { query: 'x'.repeat(201) })
  ...
  it('folds full-width query characters onto ASCII text via NFKC', async () => {
    const ctx = await boot()
    const session = Session.create(SessionId('remind-nfkc'), [
    ...
    const result = await remind(ctx, owner, { query: 'ａｂｃ' })
    expect(result.isError).toBe(false)
  ...
  it('fast path: an NFKC-stable query misses full-width variants in event text', async () => {
    const ctx = await boot()
    const session = Session.create(SessionId('remind-fastpath'), [
      ...
      { type: 'compaction/start', seq: 1, time: 2, data: { compactionId: CompactionId('remind-fastpath'), turn: null } },
    ])
    const owner = agent(ctx, session)
    const result = await remind(ctx, owner, { query: 'abc' })
    expect(result.isError).toBe(false)
    const text = resultText(result)
